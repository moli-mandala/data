# Source ingestion checklist — 20260828-zide-sora-juray

- Installed input: `data/other/forms/20260828-zide-sora-juray.csv`
...
- Installed rows: 2057
- Compiled rows carrying this unit's citation keys: 2057
- Input rows with checked grammatical evidence: 70
- Compiled rows with canonical grammatical tags: 70
- Source keys: zide1982reconstruction

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: zide1982reconstruction; 2057 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/zide_sora_gorum_1982.py
- [x] 3. Plan the installed files and identifiers — 2057 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 2 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 2057}; blank forms 0
- [x] 6. Parse structured linguistic information — 70 input rows carry checked grammatical evidence; 70 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/zide-sora-juray.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-zide-sora-juray-audit.csv, data/other/forms/raw_data/20260828-zide-sora-juray-sample.csv, data/other/forms/raw_data/20260828-zide-sora-juray-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_zide_sora_gorum_1982.py, tests/test_source_checklists.py
...
- Counts: 2057 installed records; 2057 compiled citation attestations.
- Exclusions: none; all 1,750 structured Sora and Juray source records yield 2,057 alternant-expanded form rows.
- Unresolved cases: the index exposes no protoforms; all 1,011 source comparison groups are preserved in Cognateset, but their forms remain graph-unlinked and receive no conjectural Sora–Gorum or Proto-Munda ancestor.
- Transcription: `conversion/zide-sora-juray.txt` identity-preserves NFC source Unicode; top-level commas and semicolons split alternants while punctuation inside parentheses remains intact.
...
- Representative app entries: `/references/zide1982reconstruction`, canonical Sora and Juray language pages, paired group Z82-p461-i1462 `lʌŋ`/`R-lʌŋ` 'voice', and singleton comparison groups.
