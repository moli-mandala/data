# Source ingestion checklist — 20260819-emeneau-brahui-1997

- Installed input: `data/other/forms/20260819-emeneau-brahui-1997.csv`
...
- Installed rows: 19
- Compiled rows carrying this unit's citation keys: 19
- Input rows with checked grammatical evidence: 19
- Compiled rows with canonical grammatical tags: 19
- Source keys: emeneau1997brahui

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: emeneau1997brahui; 19 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/emeneau_brahui_1997.py
- [x] 3. Plan the installed files and identifiers — 19 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 2 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 19}; blank forms 0
- [x] 6. Parse structured linguistic information — 19 input rows carry checked grammatical evidence; 19 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/emeneau-brahui.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260819-emeneau-brahui-1997-audit.csv, data/other/forms/raw_data/20260819-emeneau-brahui-1997-sample.csv, data/other/forms/raw_data/20260819-emeneau-brahui-1997-manifest.json, data/other/forms/raw_data/20260819-emeneau-brahui-1997-reconciliation.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_emeneau_brahui_1997.py, tests/test_source_checklists.py
...
- Counts: 19 installed records; 19 compiled citation attestations.
- Exclusions: the p. 440 introduction and p. 447 reference continuation yield no independent lexical rows; supporting examples and repeated cross-page claims remain accounted for in the 76-record audit rather than becoming duplicate forms.
- Unresolved cases: six source forms remain unlinked: five retain only ranked hypotheses (pužža, kūžing, pisfing, šupping, dūī) and the homonymous 'turn sour' sense of taṛifing has no proposed etymology; all 18 page-agent corrections are explicit.
- Transcription: `conversion/emeneau-brahui.txt`; Emeneau's underlined gh is preserved in Original and mapped to display ɣ, while vowel length and Dravidianist diacritics are retained.
...
- Representative app entries: `/entries/f_6voa4fsbvujpc` (bēɣ-), `/entries/f_5uv343fuclkso` (ranked kūžing hypotheses), `/entries/f_rpyanync5ohwc` (borrowed dū), `/entries/d701` ((h)ullī reassignment), and `/references/emeneau1997brahui`.
