# Source ingestion checklist — 20260824-buddruss-wama

- Installed input: `data/other/forms/20260824-buddruss-wama.csv`
...
- Installed rows: 276
- Compiled rows carrying this unit's citation keys: 276
- Input rows with checked grammatical evidence: 276
- Compiled rows with canonical grammatical tags: 276
- Source keys: buddruss-wama2006

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: buddruss-wama2006; 276 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/buddruss_wama_2006.py
- [x] 3. Plan the installed files and identifiers — 276 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 276}; blank forms 0
- [x] 6. Parse structured linguistic information — 276 input rows carry checked grammatical evidence; 276 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/buddruss-wama.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260824-buddruss-wama-audit.csv, data/other/forms/raw_data/20260824-buddruss-wama-sample.csv, data/other/forms/raw_data/20260824-buddruss-wama-manifest.json, source_checklists/installed-record-audit.csv.gz
...
- Counts: 276 installed records; 276 compiled citation attestations.
- Exclusions: the three running texts and translations, inflected examples not promoted to headword status, and comparative Ashkun, Nuristani Kalasha, Dameli, Kati, Prasun, Pashai, and other-language forms remain source prose.
- Unresolved cases: no transcription uncertainty remains in the 276 emitted headword records; the source's explicit uncertainty about the phonemic status of vowel quantity is preserved rather than normalized away.
- Transcription: all 276 records were manually collated against 400 dpi renders after two Tesseract layout passes; `conversion/buddruss-wama.txt` preserves printed vowels and maps c / č / š / ž to house ʦ / c / ś / ź.
...
- Representative app entries: `/references/buddruss-wama2006`, canonical Ashkun (`Ash`), and the registered Wama dialect (`cdial-Ash-wama`).
