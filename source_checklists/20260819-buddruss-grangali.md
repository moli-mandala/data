# Source ingestion checklist — 20260819-buddruss-grangali

- Installed input: `data/other/forms/20260819-buddruss-grangali.csv`
...
- Installed rows: 320
- Compiled rows carrying this unit's citation keys: 320
- Input rows with checked grammatical evidence: 268
- Compiled rows with canonical grammatical tags: 268
- Source keys: buddruss-grangali1979

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: buddruss-grangali1979; 320 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/buddruss_grangali_1979.py
- [x] 3. Plan the installed files and identifiers — 320 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 3 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 320}; blank forms 0
- [x] 6. Parse structured linguistic information — 268 input rows carry checked grammatical evidence; 268 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/buddruss-grangali.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260819-buddruss-grangali-audit.csv, data/other/forms/raw_data/20260819-buddruss-grangali-sample.csv, data/other/forms/raw_data/20260819-buddruss-grangali-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_buddruss_grangali_1979.py, tests/test_source_checklists.py
...
- Counts: 320 installed records; 320 compiled citation attestations.
- Exclusions: items 47, 110, and 166 are explicitly unattested; bare Ningalami/Shumashti abbreviations with no printed form and unnumbered phonological examples are excluded.
- Unresolved cases: no transcription uncertainty remains after a 323-record manual census; item 150 preserves Buddruss's heel versus Grjunberg's ankle disagreement, and item 24's loan status is secure while its proposed Pashto source remains tentative.
- Transcription: all 323 records were manually collated against the 300 dpi scan: 170 Grangali, 59 Ningalami, 91 Shumashti, and three Grangali non-attestations; `conversion/buddruss-grangali.txt` preserves Original while mapping Buddruss's explicit dental c / palatal č / retroflex c̣ contrast to ʦ / c / ʦ̣.
...
- Representative app entries: `/references/buddruss-grangali1979`, plus the independently registered language pages for Grangali (`Gng`), Ningalami (`Ning`), and Shumashti (`Shum`).
