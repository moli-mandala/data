# Source ingestion checklist — 20260824-buddruss-waigali

- Installed input: `data/other/forms/20260824-buddruss-waigali.csv`
...
- Installed rows: 158
- Compiled rows carrying this unit's citation keys: 158
- Input rows with checked grammatical evidence: 158
- Compiled rows with canonical grammatical tags: 158
- Source keys: buddruss-waigali1992

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: buddruss-waigali1992; 158 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/buddruss_waigali_1992.py
- [x] 3. Plan the installed files and identifiers — 158 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 158}; blank forms 0
- [x] 6. Parse structured linguistic information — 158 input rows carry checked grammatical evidence; 158 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/buddruss-waigali.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260824-buddruss-waigali-audit.csv, data/other/forms/raw_data/20260824-buddruss-waigali-sample.csv, data/other/forms/raw_data/20260824-buddruss-waigali-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_buddruss_waigali_wama.py, tests/test_source_checklists.py
...
- Counts: 158 installed records; 158 compiled citation attestations.
- Exclusions: the 25 proverb texts and running translations, inflected examples not promoted to headword status, and comparative forms from Kamviri, Wamai, Tregami, Pashai, and other languages remain source prose rather than independent Waigali attestations.
- Unresolved cases: no transcription uncertainty remains in the 158 emitted headword records; hedged, alternative, and secondary etymological comparisons remain prose.
- Transcription: all 158 records were manually collated against 400 dpi renders after comparison with the embedded text layer and two Tesseract passes; `conversion/buddruss-waigali.txt` preserves Original while mapping č / ǰ / š to house c / j / ś.
...
- Representative app entries: `/references/buddruss-waigali1992`, canonical Waigali (`Wg`), and the registered Nisheigram dialect (`nis`).
