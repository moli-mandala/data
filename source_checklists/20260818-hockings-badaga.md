# Source ingestion checklist — 20260818-hockings-badaga

- Installed input: `data/other/forms/20260818-hockings-badaga.csv`
...
- Installed rows: 16706
- Compiled rows carrying this unit's citation keys: 16706
- Source keys: hockings-pilotraichoor1992

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: hockings-pilotraichoor1992; 16706 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/hockings_badaga.py
- [x] 3. Plan the installed files and identifiers — 16706 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 16706}; blank forms 0
- [x] 6. Parse structured linguistic information — compiled rows are covered by global tag, schema, and false-positive tests
- [x] 7. Build and verify the sound profile — profile route: conversion/badaga-hockings.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260818-hockings-badaga-audit.csv, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_hockings_badaga.py, tests/test_source_checklists.py
- [x] 12. Install and run the full data pipeline — repository-wide results: source_checklists/VALIDATION.md
- [x] 13. Build and inspect the browser database — browser build and representative QA: source_checklists/VALIDATION.md
...
- Counts: 16706 installed records; 16706 compiled citation attestations.
- Exclusions: front matter, blank leaves, the English-Badaga reverse glossary, appendices, references, and publisher advertisement are outside the 9,993-article Badaga-English scope; no lexical article was structurally corrupt.
- Unresolved cases: 93 articles retain unresolved printed DEDR citations without conjectural links; 20 articles are image-reviewed and the remaining 9,973 retain a typed transcription-review marker.
- Transcription: `conversion/badaga-hockings.txt` converts source vowel-length colons to display macrons while preserving Original; durable scan-backed decisions live in `20260818-hockings-badaga-corrections.csv`.
- Validation: final full data/browser validation is recorded centrally in `source_checklists/VALIDATION.md`.
- Representative app entries: `/references/hockings-pilotraichoor1992`, `/entries/f_hmjkffhyzp44y` (reviewed agaṭu madilu), `/entries/f_id7i2lzuvr7ec` (review-pending Edekādu), and `/languages/Badaga`.
...
- [x] Focused importer tests, registry/profile tests, the complete data build, and the full test
      suite pass.
- [x] The browser database builds, passes its integrity/size checks, and representative entry,
...
- [x] Check the worktree first. Do not overwrite unrelated importer or generated-CLDF work in a
  shared checkout.
- [x] Run the importer without installation when supported, inspect the proposed CSV/audit, then
...
- [x] Run the complete current pipeline in order:
...
- [x] Inspect `errors.txt`. Prove the new source contributed no unmapped forms; do not hide new
  errors among older known conversion issues.
- [x] Review generated diffs and count changes in at least:
  - `cldf/forms.csv`
  - `cldf/edges.csv`
  ...
  - `cldf/alignments.csv`
  - `cldf/references.csv`
- [x] Investigate unexpectedly large ID churn, unrelated row-count changes, lost citations, or
...
- [x] Confirm the source-specific assertions against compiled CLDF, not only the raw importer
  output.

## 13. Build and inspect the browser database

- [x] Build the browser database from the sibling CLDF:
...
- [x] Confirm the builder's SQLite integrity and compact-database size guards pass.
- [x] For local inspection, stage and serve the new database:
  ...
  replaced.
- [x] Inspect representative pages in the app:
  - one ordinary entry;
...
- [x] Inspect at least one example that combines several ingestion features; record it in the
  handoff so another person can verify the work quickly.
- [x] Check that independently sourced, identical unlinked forms merge across dialects and
...
- [x] Separate headword, alternate/S2 stem, sense number, POS/gender, definition, citations,
  native script, etymology, and donor labels.
- [x] Preserve homographs with distinct `Entry_Key` values.
- [x] Emit printed alternates as variants only when the source treats them as such.
- [x] Resolve cited dictionary IDs; retain invalid citations in the audit.

### Survey wordlists or comparative tables

- [x] Identify target lects versus controls and exclude controls deliberately.
- [x] Model sites as dialects beneath existing base languages.
- [x] Preserve prompt/concept IDs so identical short forms under different prompts are not
  collapsed.
- [x] Keep IPA/source transcription and locations, with appropriate coordinate caveats.

### OCR-heavy source

- [x] Preserve raw OCR and page images/intermediates needed for review.
- [x] Compare duplicate scans or alternate OCR passes when available.
- [x] Distinguish mechanical, source-wide OCR repairs from linguistic emendation.
- [x] Mark every unreviewed/uncertain row and account for illegible blank heads.

### Website/API or external CLDF

- [x] Pin an immutable release/snapshot and record its licence.
- [x] Preserve upstream IDs, source spelling, IPA/segments, concepts, languages, and raw labels.
- [x] Cache or audit enough upstream data to reproduce the import after the live source changes.
- [x] Map upstream languages to Jambu base languages and dialects explicitly; never bulk-create
  source-prefixed top-level languages as a shortcut.

### Etymological/comparative source

- [x] Separate source comparison prose from resolvable typed graph relations.
- [x] Treat printed Turner/CDIAL/DEDR IDs as claims with their source caveats, not independent
  confirmation.
- [x] Keep uncertain alternatives visible and ranked rather than replacing the accepted analysis.
- [x] Randomly audit raw prose against forms, glosses, citations, tags, and edges after the full
