# Source ingestion checklist — 20260819-burrow-emeneau-den1

- Installed input: `data/other/forms/20260819-burrow-emeneau-den1.csv`
...
- Installed rows: 709
- Compiled rows carrying this unit's citation keys: 718
- Input rows with checked grammatical evidence: 65
- Compiled rows with canonical grammatical tags: 299
- Source keys: burrow-emeneau1972den1

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: burrow-emeneau1972den1; 709 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/burrow_emeneau_1972_den1.py
- [x] 3. Plan the installed files and identifiers — 709 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 46 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 709}; blank forms 0
- [x] 6. Parse structured linguistic information — 65 input rows carry checked grammatical evidence; 299 compiled rows carry canonical grammatical tags
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260819-burrow-emeneau-den1-audit.csv, data/other/forms/raw_data/20260819-burrow-emeneau-den1-sample.csv, data/other/forms/raw_data/20260819-burrow-emeneau-den1-manifest.json, data/other/forms/raw_data/20260819-burrow-emeneau-den1-reconciliation.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_burrow_emeneau_1972_den1.py, tests/test_source_checklists.py
...
- Counts: 709 installed records; 718 compiled citation attestations.
- Exclusions: of 1,324 nested page-agent form candidates, 709 active/corrected forms are installed after independent DEDR corroboration; 153 comparison-only, 88 queried, 43 deleted, 10 loan, 8 active/corrected non-reflex, 2 duplicate, 304 transcription-unreconciled, 6 split-target-unresolved, and 1 variant-split-pending candidate remain audit-only.
- Unresolved cases: the 304 non-uniquely corroborated transcriptions, six ambiguous current-DEDR descendants, one combined variant field, and all 1,154 page-agent running-text segments await diplomatic image review; no unreviewed prose is published.
- Transcription: `conversion/dedr.txt`; source strings are routed through the DED profile only after exact or unique diacritic-insensitive current-DEDR corroboration, with every agent correction retained in the audit; 286 installed forms use an unambiguous registered dialect ID while source sigla and mixed-dialect labels remain at base-language level.
...
- Representative app entries: `/entries/d512` (old 435 iḷusan), `/entries/d811` (old 694 talay-ēru), `/entries/d800` (old 2127 jicoṇa), `/entries/d4556` (old 3722 boḷi), and `/references/burrow-emeneau1972den1`.
