# Source ingestion checklist — 20260828-sil-haryanvi

- Installed input: `data/other/forms/20260828-sil-haryanvi.csv`
...
- Installed rows: 1553
- Compiled rows carrying this unit's citation keys: 1553
- Input rows with checked grammatical evidence: 72
- Compiled rows with canonical grammatical tags: 72
- Source keys: webster2024haryanvi

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: webster2024haryanvi; 1553 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_haryanvi_2024/extract_ocr.py, data/other/forms/raw_data/sil_haryanvi_2024/import_haryanvi.py
- [x] 3. Plan the installed files and identifiers — 1553 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 1553}; blank forms 0
- [x] 6. Parse structured linguistic information — 72 input rows carry checked grammatical evidence; 72 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-haryanvi.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_haryanvi_2024/manual_transcription.tsv, data/other/forms/raw_data/20260828-sil-haryanvi-audit.csv, data/other/forms/raw_data/20260828-sil-haryanvi-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_haryanvi_2024.py, tests/test_source_checklists.py
...
- Counts: 1553 installed records; 1553 compiled citation attestations.
- Exclusions: 21 visibly blank target cells, one elicitation-only target cell, and 840 Braj/Haryanvi, Baghati Pahari, Hindustani and Punjabi comparison cells are audit-only; 1,553 variants from six Haryanvi lists install.
- Unresolved cases: typed audit notes preserve faint, unlabelled, repeated-label and clipped source fragments; seven same-list cross-references are resolved without guessing.
- Transcription: `conversion/sil-haryanvi.txt`; structural OCR supplied comparison evidence only, and all 1,260 target cells were manually inspected on enlarged source crops.
- Validation: seven focused source/import/profile/metadata checks pass; consolidated full build and browser QA remain deferred until the parallel survey batch lands.
- Representative app entries: Rohtak item 1 `d̪eh`, Jind item 4 `mu`, Fatehabad item 52 alternate `d̪ʌg·ʌl`, and Fatehabad item 209 resolved cross-reference `t̪u`.
