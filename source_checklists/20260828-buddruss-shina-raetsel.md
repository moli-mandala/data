# Source ingestion checklist — 20260828-buddruss-shina-raetsel

- Installed input: `data/other/forms/20260828-buddruss-shina-raetsel.csv`
...
- Installed rows: 311
- Compiled rows carrying this unit's citation keys: 311
- Input rows with checked grammatical evidence: 311
- Compiled rows with canonical grammatical tags: 311
- Source keys: buddruss-shina1996

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: buddruss-shina1996; 311 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/buddruss_shina_1996.py
- [x] 3. Plan the installed files and identifiers — 311 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 311}; blank forms 0
- [x] 6. Parse structured linguistic information — 311 input rows carry checked grammatical evidence; 311 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/buddruss-shina.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-buddruss-shina-raetsel-audit.csv, data/other/forms/raw_data/20260828-buddruss-shina-raetsel-sample.csv, data/other/forms/raw_data/20260828-buddruss-shina-raetsel-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_buddruss_shina_1996.py, tests/test_source_checklists.py
...
- Counts: 311 installed records; 311 compiled citation attestations.
- Exclusions: 58 running riddles and translations, inflected examples not promoted to headword status, comparison-only non-Shina forms, the bibliography, and the closing summary are excluded; all 296 analytical glossary units are represented.
- Unresolved cases: wáaku remains explicitly unintelligible as in the source; questioned, competing, component-only, and comparison-only Turner claims remain unlinked rather than receiving conjectural CDIAL assignments.
- Transcription: `conversion/buddruss-shina.txt`; double vowels and mora-positioned accents preserve Buddruss's quantity and tone distinctions, while printed nasal marks use the source-profile `~` notation and Original retains the checked source spelling.
- Validation: the 311-row source-specific build and focused tests pass; the fresh browser database passes integrity and visual QA; unrelated repository-wide failures are itemized in `source_checklists/VALIDATION.md`.
- Representative app entries: `/references/buddruss-shina1996`, canonical Shina (`Sh`), registered Gilgit dialect (`gil`), linked `áa~i` 'mouth', unlinked `čhii~ṣ` 'mountain', and the `agúl`/`hagúl` cross-reference variant pair.
