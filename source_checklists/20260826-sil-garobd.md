# Source ingestion checklist — 20260826-sil-garobd

- Installed input: `data/other/forms/20260826-sil-garobd.csv`
...
- Installed rows: 4444
- Compiled rows carrying this unit's citation keys: 4444
- Input rows with checked grammatical evidence: 0
- Compiled rows with canonical grammatical tags: 0
- Source keys: kim-kim-sangma2012garo
- Full-source state: partial legacy-font recovery; 712 attested audit records still require independent visual transcription from rendered publisher pages
...
- [x] 3. Plan the installed files and identifiers — 4444 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 5 input language/lect IDs; registry gaps: none
- [ ] 5. Emit the rich import schema — the decoded rows have the rich schema, but 712 otherwise attested records are omitted pending manual transcription
- [x] 6. Parse structured linguistic information — no source-supplied grammatical labels detected by the scoped parser
- [ ] 7. Build and verify the sound profile — complete input-symbol coverage cannot be asserted until the omitted attestations are manually recovered
...
- [ ] 10. Produce a complete audit trail — the legacy audit pins every omission, but an independent visual-review ledger and source-local manual manifest are pending
...
- Counts: 4444 installed records; 4444 compiled citation attestations.
- Exclusions: 91 per-site printed gaps and 17 globally unused items are audit-only.
- Unresolved cases: 712 attested records contain at least one glyph without a verified decoder mapping; all remain audit-only until manually transcribed and rechecked from rendered pages.
- Transcription: the decoded subset has no explicit profile route; the render-first manual package is queued in `data/other/forms/raw_data/sil_bangladesh_legacy_manual_queue.md`.
- Validation: exact partial-decoder disposition counts are guarded by `tests/test_sil_bracket_wordlists.py`; manual source-local gates and the consolidated build remain pending.
- Representative app entries: recorded centrally in `source_checklists/VALIDATION.md`.

## Filled checklist copy

Checked boxes below describe only the installed subset. The source is not complete until the manual-recovery gates above pass; addenda not listed are inapplicable.
