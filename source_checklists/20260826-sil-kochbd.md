# Source ingestion checklist — 20260826-sil-kochbd

- Installed input: `data/other/forms/20260826-sil-kochbd.csv`
...
- Installed rows: 1017
- Compiled rows carrying this unit's citation keys: 1017
- Input rows with checked grammatical evidence: 0
- Compiled rows with canonical grammatical tags: 0
- Source keys: kim-ahmad-kim-sangma2011kochbd
- Full-source state: all 307 Appendix A.3 items manually reviewed and the 1,017 resolved Koch target attestations installed through shared source-specific integration; consolidated CLDF and browser gates remain deferred
...
- [x] 3. Plan the installed files and identifiers — 1017 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — 1,017 resolved target attestations use the 15-column schema and unique immutable silkochbd2011 source keys
- [x] 6. Parse structured linguistic information — no source-supplied grammatical labels detected by the scoped parser
- [x] 7. Build and verify the sound profile — conversion/sil-bangladesh.txt covers all 44 attested codepoints with zero additions, unresolved mappings or replacement characters
...
- [x] 10. Produce a complete audit trail — the 2,159-row staging audit and 2,208-row legacy reconciliation account for every target, control, ambiguity, blank, not-used and legacy-collision disposition
- [x] 11. Add focused regression tests — tests: tests/test_sil_kochbd_2011_manual.py, tests/test_sil_kochbd_2011_post_freeze.py, tests/test_sound_profiles.py, tests/test_dialects.py
- [ ] 12. Install and run the full data pipeline — shared source-specific integration complete; global source-audit regeneration, consolidated build and full-suite validation deliberately deferred
...
- Counts: 1017 installed records; 1017 compiled citation attestations.
- Exclusions: 772 resolved A'tong/Bangla control rows, 226 ambiguous expanded rows, 25 printed blanks and 119 not-used rows are audit-only.
- Unresolved cases: 225 ambiguity-only conceptual cells plus the unresolved variant at mixed item 241/site r produce 226 coordinates and 226 expanded rows with unresolved modifiers; none is guessed or installed.
- Transcription: every lexical cell was manually read from rendered physical pages 43--62; OCR, PDF text, raw legacy glyphs and installed forms were locator or post-freeze comparison only and supplied or verified no reading.
- Validation: frozen-ledger, exhaustive reconciliation, staging, registry, reference, profile routing and parser invariants are guarded by `tests/test_sil_kochbd_2011_manual.py` and `tests/test_sil_kochbd_2011_post_freeze.py`; consolidated build pending.
- Representative app entries: recorded centrally in `source_checklists/VALIDATION.md`.

## Filled checklist copy

Checked boxes describe the completed source-specific integration stage. Global source-audit regeneration, consolidated build/full-suite validation and browser QA remain deferred.
