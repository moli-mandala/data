# Source ingestion checklist — 20260828-munda-proto-kherwarian

- Installed input: `data/other/forms/20260828-munda-proto-kherwarian.csv`
...
- Installed rows: 2919
- Compiled rows carrying this unit's citation keys: 2997
- Input rows with checked grammatical evidence: 110
- Compiled rows with canonical grammatical tags: 110
- Source keys: munda1968proto

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: munda1968proto; 2919 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/munda_proto_kherwarian_1968.py
- [x] 3. Plan the installed files and identifiers — 2919 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 3 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 2919}; blank forms 0
- [x] 6. Parse structured linguistic information — 110 input rows carry checked grammatical evidence; 110 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/munda-proto-kherwarian.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-munda-proto-kherwarian-audit.csv, data/other/forms/raw_data/20260828-munda-proto-kherwarian-sample.csv, data/other/forms/raw_data/20260828-munda-proto-kherwarian-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_munda_proto_kherwarian_1968.py, tests/test_source_checklists.py
...
- Counts: 2919 installed records; 2997 compiled citation attestations.
- Exclusions: none; all 2,768 structured source records have usable Unicode forms and glosses, yielding 920 parameters and 2,919 alternant-expanded form rows.
- Unresolved cases: Santali `siɲ aɽaʔ` and pre-Mundari/Santali `ɡuɽu`/`ɡuɽɡu` have no indexed Proto-Kherwarian reconstruction and remain installed but unlinked; no Proto-Munda relationship is inferred.
- Transcription: `conversion/munda-proto-kherwarian.txt` identity-preserves NFC source Unicode; spaced tildes alone split alternants, while slash and optional-segment notation remain uninterpreted source evidence in Original, Form, and Phonemic.
...
- Representative app entries: `/references/munda1968proto`, Proto-Kherwarian `*(d)ɛla` with pre-Mundari and Santali `(d)ɛla` 'to come', the two `three` alternants, and unlinked grinding-stone records.
