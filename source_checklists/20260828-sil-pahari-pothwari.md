# Source ingestion checklist — 20260828-sil-pahari-pothwari

- Installed input: `data/other/forms/20260828-sil-pahari-pothwari.csv`
...
- Installed rows: 3038
- Compiled rows carrying this unit's citation keys: 3038
- Input rows with checked grammatical evidence: 0
- Compiled rows with canonical grammatical tags: 0
- Source keys: lothers-lothers2010pahari

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: lothers-lothers2010pahari; 3038 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_pahari_pothwari_2010/extract_pahari_pothwari.py, data/other/forms/raw_data/sil_pahari_pothwari_2010/import_pahari_pothwari.py
- [x] 3. Plan the installed files and identifiers — 3038 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 3038}; blank forms 0
- [x] 6. Parse structured linguistic information — no source-supplied grammatical labels detected by the scoped parser
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-pahari-pothwari.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-sil-pahari-pothwari-audit.csv, data/other/forms/raw_data/20260828-sil-pahari-pothwari-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_pahari_pothwari_2010.py, tests/test_source_checklists.py
...
- Counts: 3038 installed records; 3038 compiled citation attestations.
- Exclusions: the 434 Abbottabad and Mansehra Hindko comparison cells, including eighteen source blanks at items 209--217, are audit-only; all 3,038 responses from the fourteen Pahari, Pothwari and Mirpuri target lists install.
- Unresolved cases: no concept, list row, response cell or source symbol remains structurally unparsed; fourteen printed `AUS` labels in the invariant OSI row are retained raw and explicitly normalized to the Osia list.
- Transcription: `conversion/sil-pahari-pothwari.txt`; no OCR is used. Appendix B.1 has a complete positioned Doulos SIL text layer, and content-stream order preserves the source's Indological Phonetic Script, length, nasalisation and breathy-voice marks.
- Validation: focused extraction, full-cell topology, audit, profile, language/dialect metadata and compiled-identity tests are required; repository-wide results are recorded in VALIDATION.md.
- Representative app entries: `/references/lothers-lothers2010pahari`, exact source-diacritic search, and `/languages/poth` with fourteen ESR 2010-012 locality tags.
