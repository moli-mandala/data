# Source ingestion checklist — 20260828-bhattacharya-bonda

- Installed input: `data/other/forms/20260828-bhattacharya-bonda.csv`
...
- Installed rows: 3330
- Compiled rows carrying this unit's citation keys: 3330
- Input rows with checked grammatical evidence: 183
- Compiled rows with canonical grammatical tags: 183
- Source keys: bhattacharya1968bonda

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: bhattacharya1968bonda; 3330 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/bhattacharya_bonda_1968.py
- [x] 3. Plan the installed files and identifiers — 3330 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 3330}; blank forms 0
- [x] 6. Parse structured linguistic information — 183 input rows carry checked grammatical evidence; 183 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/bhattacharya-bonda.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-bhattacharya-bonda-audit.csv, data/other/forms/raw_data/20260828-bhattacharya-bonda-sample.csv, data/other/forms/raw_data/20260828-bhattacharya-bonda-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_bhattacharya_bonda_1968.py, tests/test_source_checklists.py
...
- Counts: 3330 installed records; 3330 compiled citation attestations.
- Exclusions: one exact repeated `da?tukui` alternant is audit-only; all 2,881 structured records remain accounted for and yield 3,330 installed rows.
- Unresolved cases: eight malformed, absent, or multiply matching printed `see` targets remain unlinked and intentionally glossless; 27 uniquely resolved references link to their source targets, and one two-target reference receives only the targets' shared gloss. Three Hill Bondo records printed without definitions retain blank glosses.
- Transcription: `conversion/bhattacharya-bonda.txt` identity-preserves NFC source Unicode; top-level commas and semicolons split alternants, question mark remains a source transcription symbol, and the three terminal `(E?)` provenance/query markers are audit-preserved but removed from Form and tagged `uncertain`.
...
- Representative app entries: `/references/bhattacharya1968bonda`, registered Plains and Hill Bondo dialect views, Plains `bɔbɔ` resolved to `babu` 'a term used to address younger ones endearingly', and the unresolved source cross-reference `bip'` → `raŋbip'`.
