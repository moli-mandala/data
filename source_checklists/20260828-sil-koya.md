# Source ingestion checklist — 20260828-sil-koya

- Installed input: `data/other/forms/20260828-sil-koya.csv`
...
- Installed rows: 1438
- Compiled rows carrying this unit's citation keys: 1153
- Input rows with checked grammatical evidence: 4
- Compiled rows with canonical grammatical tags: 3
- Source keys: devagnanavaram-et-al2021koya

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: devagnanavaram-et-al2021koya; 1438 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_koya_2021/extract_ocr.py, data/other/forms/raw_data/sil_koya_2021/import_koya.py
- [x] 3. Plan the installed files and identifiers — 1438 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 1438}; blank forms 0
- [x] 6. Parse structured linguistic information — 4 input rows carry checked grammatical evidence; 3 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-koya.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_koya_2021/manual_review_data.py, data/other/forms/raw_data/20260828-sil-koya-audit.csv, data/other/forms/raw_data/20260828-sil-koya-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_koya_2021.py, tests/test_source_checklists.py
...
- Counts: 1438 installed records; 1153 compiled citation attestations.
- Exclusions: 69 missing target slots and 420 Telugu and Oriya comparison cells are audit-only; 1,438 variants from seven Koya, Gondi and Madia lists install.
- Unresolved cases: Malakanagiri item 31 retains an explicitly flagged unclear medial source symbol; two collector question marks and all absent or clipped slots remain typed.
- Transcription: `conversion/sil-koya.txt`; OCR was comparison scaffolding only, and every one of the 1,840 printed cells was manually inspected against the source images.
- Validation: eleven focused source/import/profile/metadata checks pass; consolidated full build and browser QA remain deferred until the parallel survey batch lands.
- Representative app entries: Jaganathapuram body `oleu`, Chintoor heart `gundikaia`, Utnoor body `mɛːnd̪ol`, and Malakanagiri heart `dʒiːva`.
