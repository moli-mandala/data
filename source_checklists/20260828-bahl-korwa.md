# Source ingestion checklist — 20260828-bahl-korwa

- Installed input: `data/other/forms/20260828-bahl-korwa.csv`
...
- Installed rows: 1830
- Compiled rows carrying this unit's citation keys: 1840
- Input rows with checked grammatical evidence: 35
- Compiled rows with canonical grammatical tags: 37
- Source keys: BAHL

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: BAHL; 1830 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/bahl_korwa_1962.py
- [x] 3. Plan the installed files and identifiers — 1830 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 1830}; blank forms 0
- [x] 6. Parse structured linguistic information — 35 input rows carry checked grammatical evidence; 37 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/bahl-korwa.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-bahl-korwa-audit.csv, data/other/forms/raw_data/20260828-bahl-korwa-sample.csv, data/other/forms/raw_data/20260828-bahl-korwa-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_bahl_korwa_1962.py, tests/test_source_checklists.py
...
- Counts: 1830 installed records; 1840 compiled citation attestations.
- Exclusions: one empty indexed source record is retained audit-only; all 1,791 nonempty records yield 1,830 installed form rows.
- Unresolved cases: 10 older Rau BAHL citations cannot be reconciled safely with the keyed source because the form is absent or its meaning conflicts; they remain separate legacy evidence, while 57 uniquely normalized and semantically compatible records replace their legacy excerpts.
- Transcription: `conversion/bahl-korwa.txt` identity-preserves NFC source Unicode; top-level commas split 39 source alternants, and uncertain editorial notes are retained verbatim and tagged `uncertain`.
...
- Representative app entries: `/references/BAHL`, canonical Korwa (`kw`), linked `goej`/`goeˀ` 'to die' under Proto-Munda m51, and unlinked `gadʰaː` 'donkey' with its source query.
