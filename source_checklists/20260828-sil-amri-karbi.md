# Source ingestion checklist — 20260828-sil-amri-karbi

- Installed input: `data/other/forms/20260828-sil-amri-karbi.csv`
...
- Installed rows: 5092
- Compiled rows carrying this unit's citation keys: 2092
- Input rows with checked grammatical evidence: 0
- Compiled rows with canonical grammatical tags: 0
- Source keys: abraham-daimary2021amrikarbi

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: abraham-daimary2021amrikarbi; 5092 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_amri_karbi_2021/extract_amri.py, data/other/forms/raw_data/sil_amri_karbi_2021/finalize_review.py, data/other/forms/raw_data/sil_amri_karbi_2021/import_amri.py
- [x] 3. Plan the installed files and identifiers — 5092 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 2 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 5092}; blank forms 0
- [x] 6. Parse structured linguistic information — no source-supplied grammatical labels detected by the scoped parser
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-amri-karbi.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_amri_karbi_2021/reviewed_transcription.tsv, data/other/forms/raw_data/20260828-sil-amri-karbi-audit.csv, data/other/forms/raw_data/20260828-sil-amri-karbi-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_amri_karbi_2021.py, tests/test_source_checklists.py
...
- Counts: 5092 installed records; 2092 compiled citation attestations.
- Exclusions: 631 Khasi and Assamese control responses, six explicit `no entry` cells, and 237 exact repeated target occurrences printed under multiple similarity groups are audit-only; 5,092 forms from three Amri Karbi and twelve Karbi lists install. Four reported Amri lists absent from Appendix B.3 are not reconstructed.
- Unresolved cases: no transcription is unresolved or illegible. Excluded Assamese control item 91/Z faithfully retains the source-marked uncertainty `soʌ̆ĭ??` at PDF page 59.
- Transcription: `conversion/sil-amri-karbi.txt`; Appendix B.3 is typeset Unicode, and all 5,219 conceptual cells plus all 5,966 printed records were manually compared against the 79 rendered canonical pages. No OCR was used.
- Validation: eight focused source/import/audit/profile checks pass; consolidated full build and browser QA remain deferred until the parallel survey batch lands.
- Representative app entries: Holanki item 1 `sĭnĭŋ`, Hajarongpi item 1 `sŭneŋ`, and Amguri (Kamrup) item 1 `ʌ̆ŋsoŋ`.
