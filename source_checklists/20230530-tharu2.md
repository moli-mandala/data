...
- Installed rows: 3560
- Compiled rows carrying this unit's citation keys: 3560
- Input rows with checked grammatical evidence: 528
- Compiled rows with canonical grammatical tags: 528
- Source keys: webster

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: webster; 3560 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_western_tharu_2017/import_western_tharu_2017.py
- [x] 3. Plan the installed files and identifiers — 3560 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 6 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 3560}; blank forms 0
- [x] 6. Parse structured linguistic information — 528 input rows carry checked grammatical evidence; 528 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-western-tharu.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_western_tharu_2017/staged_audit.tsv, data/other/forms/raw_data/sil_western_tharu_2017/unresolved_readings.tsv, data/other/forms/raw_data/sil_western_tharu_2017/source_manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_tharu.py, data/other/forms/raw_data/sil_western_tharu_2017/test_western_tharu_2017.py
...
- Counts: 3560 installed records; 3560 compiled citation attestations.
- Exclusions: 98 target source-blank cells and all 210 Standard Hindi comparison cells are audit-only; 3,560 target response occurrences from 3,052 attested target cells install.
- Unresolved cases: all 420 conceptual cells belonging to the two duplicate-code RNS lists retain the typed locality-assignment uncertainty and exact coordinates in the source-local audit; there are no ambiguous or illegible lexical readings.
- Transcription: `conversion/sil-western-tharu.txt` is an explicit preservation profile for the rendered-page, hand-keyed IPA; PDF text, OCR, and the retired legacy CSV supplied no reading.
- Validation: source-local staging and focused importer/registry/profile tests pass; consolidated CLDF/full-suite validation is deliberately deferred.
- Representative app entries: deferred until the consolidated CLDF and browser database are rebuilt.
