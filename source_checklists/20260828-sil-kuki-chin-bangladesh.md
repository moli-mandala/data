# Source ingestion checklist — 20260828-sil-kuki-chin-bangladesh

- Installed input: `data/other/forms/20260828-sil-kuki-chin-bangladesh.csv`
...
- Installed rows: 3235
- Compiled rows carrying this unit's citation keys: 3235
- Input rows with checked grammatical evidence: 0
- Compiled rows with canonical grammatical tags: 0
- Source keys: kim-roy-sangma2011kukichin

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: kim-roy-sangma2011kukichin; 3235 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_kuki_chin_bangladesh_2011/extract_kuki_chin.py, data/other/forms/raw_data/sil_kuki_chin_bangladesh_2011/import_kuki_chin.py
- [x] 3. Plan the installed files and identifiers — 3235 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 5 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 3235}; blank forms 0
- [x] 6. Parse structured linguistic information — no source-supplied grammatical labels detected by the scoped parser
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-bangladesh.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-sil-kuki-chin-bangladesh-audit.csv, data/other/forms/raw_data/20260828-sil-kuki-chin-bangladesh-manifest.json, data/other/forms/raw_data/sil_kuki_chin_bangladesh_2011/sag_ipa_used.tsv, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_kuki_chin_bangladesh_2011.py, tests/test_source_checklists.py
...
- Counts: 3235 installed records; 3235 compiled citation attestations.
- Exclusions: 307 standard Bangla responses and 333 external Myanmar Khumi comparison attestations are audit-only; 3,235 responses from ten Bangladesh lists install.
- Unresolved cases: no private-use code point or source line remains unmapped or unparsed; all 53 explicit `no entry` records occur in the audit-only Myanmar Khumi comparison.
- Transcription: `conversion/sil-bangladesh.txt`; all 16,029 SAG legacy-font occurrences were decoded from embedded font outlines and the exact 65 used glyphs are pinned.
- Validation: seven focused source checks, four relevant profile/build-routing checks and eight browser-database unit tests pass; full build reaches references before the two unrelated stale Kannauji assertions recorded in VALIDATION.md.
- Representative app entries: `/languages/Pangkhua` plus BawmChin, AshoChin, KhumiChin and Mizo; `/references/kim-roy-sangma2011kukichin`; `/entries/f_hc6fe7vkp3xj2`; exact `rɨvan` search; and expanded `/concepts/1732` (SKY) evidence.
