# Source ingestion checklist — 20260818-nured-org

- Installed input: `data/other/forms/20260818-nured-org.csv`
...
- Installed rows: 263
- Compiled rows carrying this unit's citation keys: 281
- Input rows with checked grammatical evidence: 263
- Compiled rows with canonical grammatical tags: 263
- Source keys: nured

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: nured; 263 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/nured_org.py
- [x] 3. Plan the installed files and identifiers — 263 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 6 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 263}; blank forms 0
- [x] 6. Parse structured linguistic information — 263 input rows carry checked grammatical evidence; 263 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/nured.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260818-nured-org-audit.csv, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_nured_org.py, tests/test_source_checklists.py
...
- Counts: 263 installed records; 281 compiled citation attestations.
- Exclusions: 770 hard redirects are excluded before fetch; of 105 nonredirect pages, 58 site or reference pages are outside scope; early spellings, untemplated examples, source language forms, and non-commentary article sections remain in the per-page audit.
- Unresolved cases: none in the installed scope: all 47 lexical articles route to a PNur entry and all 255 explicit Nuristani Form templates parse; 18 stable PNur heads are generated where no compatible existing sibling is available.
- Transcription: `conversion/nured.txt` losslessly preserves the source's diacritized Nuristani forms; 24 source variety labels are registered as language-qualified dialect tags.
...
- Representative app entries: the generated PNur borrowing from page 226, the existing two-branch barley routing from page 169, the semantically selected PNur branch from page 1082, and `/references/nured`.
