# Source ingestion checklist — 20260828-pinnow-juang

- Installed input: `data/other/forms/20260828-pinnow-juang.csv`
...
- Installed rows: 1818
- Compiled rows carrying this unit's citation keys: 1825
- Input rows with checked grammatical evidence: 285
- Compiled rows with canonical grammatical tags: 287
- Source keys: PJDW

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: PJDW; 1818 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/pinnow_juang_1960.py
- [x] 3. Plan the installed files and identifiers — 1818 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 1818}; blank forms 0
- [x] 6. Parse structured linguistic information — 285 input rows carry checked grammatical evidence; 287 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/pinnow-juang.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260828-pinnow-juang-audit.csv, data/other/forms/raw_data/20260828-pinnow-juang-sample.csv, data/other/forms/raw_data/20260828-pinnow-juang-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_pinnow_juang_1960.py, tests/test_source_checklists.py
...
- Counts: 1818 installed records; 1825 compiled citation attestations.
- Exclusions: six exactly repeated alternants are retained audit-only; all 1,658 source records remain accounted for and yield 1,818 installed rows.
- Unresolved cases: 185 records printed with no gloss or only `?` remain intentionally glossless; seven older Rau PJDW citations cannot be reconciled safely and remain separate legacy evidence, while 66 source records replace secure legacy excerpts.
- Transcription: `conversion/pinnow-juang.txt` identity-preserves NFC source Unicode; terminal `??` editorial markers are retained in the audit, removed from Form, and tagged `uncertain`; two terminal Elwin/source markers move to Notes so their commas do not become false forms; underscores in gloss phrases become spaces.
...
- Representative app entries: `/references/PJDW`, canonical Juang (`ju`), linked `elaŋ`/`ɛlaŋ` 'tongue' under Proto-Munda m3, and the intentionally glossless `hakɔb-ɖag`.
