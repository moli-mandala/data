# Source ingestion checklist — 20260828-sil-korku

- Installed input: `data/other/forms/20260828-sil-korku.csv`
...
- Installed rows: 1521
- Compiled rows carrying this unit's citation keys: 967
- Input rows with checked grammatical evidence: 50
- Compiled rows with canonical grammatical tags: 39
- Source keys: stahl2021korku

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: stahl2021korku; 1521 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_korku_2021/extract_ocr.py, data/other/forms/raw_data/sil_korku_2021/import_korku.py
- [x] 3. Plan the installed files and identifiers — 1521 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 1521}; blank forms 0
- [x] 6. Parse structured linguistic information — 50 input rows carry checked grammatical evidence; 39 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-korku.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_korku_2021/manual_review_data.py, data/other/forms/raw_data/20260828-sil-korku-audit.csv, data/other/forms/raw_data/20260828-sil-korku-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_korku_2021.py, tests/test_source_checklists.py
...
- Counts: 1521 installed records; 967 compiled citation attestations.
- Exclusions: 216 confirmed blank target cells, one illegible/clipped target cell, and all 210 Nihali comparison cells are audit-only; 1,463 attested Korku cells yield 1,521 installed rows after source slash alternatives are expanded.
- Unresolved cases: Amdhana Mawasi item 93 `tail` at PDF page 83 / printed page 78 contains only faint clipped marks and is excluded without guessing; 24 collector-question cases and 16 faint/clipped/ambiguous but readable forms retain typed flags.
- Transcription: `conversion/sil-korku.txt`; OCR is a separate comparison scaffold only. Every one of the 1,890 image-only cells (1,680 target and 210 control) was manually read and checked against the canonical scan.
- Validation: nine focused source/import/audit/profile checks pass; consolidated full build and browser QA remain deferred until the parallel survey batch lands.
- Representative app entries: Chikli Ruma body `kombor`, Lahi head alternatives `dẽi` / `kapar`, and Khamalpur Bondoy item 18 `dʒaɖa`.
