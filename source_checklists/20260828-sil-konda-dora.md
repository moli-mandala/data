# Source ingestion checklist — 20260828-sil-konda-dora

- Installed input: `data/other/forms/20260828-sil-konda-dora.csv`
...
- Installed rows: 452
- Compiled rows carrying this unit's citation keys: 289
- Input rows with checked grammatical evidence: 0
- Compiled rows with canonical grammatical tags: 0
- Source keys: blair-george2012kondadora

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: blair-george2012kondadora; 452 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_konda_dora_2012/extract_scaffold.py, data/other/forms/raw_data/sil_konda_dora_2012/import_konda_dora.py
- [x] 3. Plan the installed files and identifiers — 452 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 452}; blank forms 0
- [x] 6. Parse structured linguistic information — no source-supplied grammatical labels detected by the scoped parser
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-konda-dora.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_konda_dora_2012/reviewed_transcription.psv, data/other/forms/raw_data/20260828-sil-konda-dora-audit.csv, data/other/forms/raw_data/20260828-sil-konda-dora-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_konda_dora_2012.py, tests/test_source_checklists.py
...
- Counts: 452 installed records; 289 compiled citation attestations.
- Exclusions: 43 confirmed blank target cells, 86 confirmed blank control cells, and 342 attested Telugu and Adivasi Oriya control cells are audit-only; 385 attested Konda cells yield 452 installed rows after source-defined slash expansion.
- Unresolved cases: no cell is ambiguous, clipped, illegible, source-questioned, or unresolved. The report's duplicate item number 212 is preserved as separate liver and foot prompts.
- Transcription: `conversion/sil-konda-dora.txt`; all 856 image-scan cells (428 target and 428 controls) were manually transcribed and checked from rendered pages. The corrupt text/OCR layer is a locator scaffold only and never supplies an accepted reading.
- Validation: seven focused source/import/audit/profile checks pass; consolidated full build and browser QA remain deferred until the parallel survey batch lands.
- Representative app entries: Koraput body `oɽol`, Koraput item 74 `bumi ka:ʈoliŋ`, and Visakh item 122 `i?en`, with literal source punctuation preserved diplomatically.
