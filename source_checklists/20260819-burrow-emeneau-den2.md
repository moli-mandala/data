# Source ingestion checklist — 20260819-burrow-emeneau-den2

- Installed input: `data/other/forms/20260819-burrow-emeneau-den2.csv`
...
- Installed rows: 159
- Compiled rows carrying this unit's citation keys: 178
- Input rows with checked grammatical evidence: 7
- Compiled rows with canonical grammatical tags: 80
- Source keys: burrow-emeneau1972den2

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: burrow-emeneau1972den2; 159 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/burrow_emeneau_1972_den2.py
- [x] 3. Plan the installed files and identifiers — 159 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 32 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 159}; blank forms 0
- [x] 6. Parse structured linguistic information — 7 input rows carry checked grammatical evidence; 80 compiled rows carry canonical grammatical tags
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/20260819-burrow-emeneau-den2-audit.csv, data/other/forms/raw_data/20260819-burrow-emeneau-den2-sample.csv, data/other/forms/raw_data/20260819-burrow-emeneau-den2-manifest.json, data/other/forms/raw_data/20260819-burrow-emeneau-den2-reconciliation.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_burrow_emeneau_1972_den2.py, tests/test_source_checklists.py
...
- Counts: 159 installed records; 178 compiled citation attestations.
- Exclusions: of 448 split page-agent form candidates, 159 DEDS forms are installed after independent current-DEDR language/form/gloss corroboration; 20 comparison-only, 28 queried, 14 deleted, 3 loan, 1 active borrowed, 46 transcription-unreconciled, 25 DEDS target-unresolved, and 152 DBIA loan-entry-pending candidates remain audit-only.
- Unresolved cases: the 46 uncorroborated DEDS transcriptions, 25 DEDS forms without a current target, all 152 active DBIA additions/corrections, and all 119 page-agent running-text segments await their applicable diplomatic or loan-entry review; no unreviewed prose or DBIA form is published.
- Transcription: `conversion/dedr.txt`; printed S² labels are treated as DEN-II new-entry numbers, not historical DEDS IDs, and forms are routed only after current-DEDR language/form/gloss corroboration; 39 installed forms use a registered dialect ID.
...
- Representative app entries: `/entries/d49` (S²1 accu), `/entries/d2121` (S²28 koyk), `/entries/d2728` (S²37 sūri), `/entries/d3523` (S²46 tōṛa), `/entries/d4375` (S²65 pu·ḷï 'mist', not the d4322 homonym), and `/references/burrow-emeneau1972den2`.
