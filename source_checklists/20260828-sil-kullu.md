# Source ingestion checklist — 20260828-sil-kullu

- Installed input: `data/other/forms/20260828-sil-kullu.csv`
...
- Installed rows: 2963
- Compiled rows carrying this unit's citation keys: 1725
- Input rows with checked grammatical evidence: 5
- Compiled rows with canonical grammatical tags: 4
- Source keys: blair2021kullu

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: blair2021kullu; 2963 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_kullu_2021/extract_kullu.py, data/other/forms/raw_data/sil_kullu_2021/import_kullu.py
- [x] 3. Plan the installed files and identifiers — 2963 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 2963}; blank forms 0
- [x] 6. Parse structured linguistic information — 5 input rows carry checked grammatical evidence; 4 compiled rows carry canonical grammatical tags
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-kullu.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_kullu_2021/manual_pages.tsv, data/other/forms/raw_data/20260828-sil-kullu-audit.csv, data/other/forms/raw_data/20260828-sil-kullu-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_kullu_2021.py, tests/test_source_checklists.py
...
- Counts: 2963 installed records; 1725 compiled citation attestations.
- Exclusions: 415 visibly blank response cells and one printed Hindi gutter label are audit-only; 2,963 variants from sixteen Kullui-area lists install.
- Unresolved cases: Chinninal item 192 and faint Bathad item 193 remain explicitly uncertain; three source question marks are preserved and no unread form is guessed.
- Transcription: `conversion/sil-kullu.txt`; OCR was comparison scaffolding only, and all 3,168 handwritten response cells were manually inspected against source images.
- Validation: ten focused source/import/profile/metadata checks pass; consolidated full build and browser QA remain deferred until the parallel survey batch lands.
- Representative app entries: Churla body `dʒɪsəm`, Jibhi yes alternatives `hã` / `oː`, and Ani item 198 `t̪eɽə nʊ kidʒi əsə`.
