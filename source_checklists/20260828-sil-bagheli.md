# Source ingestion checklist — 20260828-sil-bagheli

- Installed input: `data/other/forms/20260828-sil-bagheli.csv`
...
- Installed rows: 5828
- Compiled rows carrying this unit's citation keys: 1628
- Input rows with checked grammatical evidence: 0
- Compiled rows with canonical grammatical tags: 0
- Source keys: koshy2022bagheli

## Retrospective gate assessment

- [x] 1. Establish the source and scope — source keys: koshy2022bagheli; 5828 installed records
- [x] 2. Choose the extraction path — importer/raw route: data/other/forms/raw_data/sil_bagheli_2022/extract_ocr.py, data/other/forms/raw_data/sil_bagheli_2022/import_bagheli.py
- [x] 3. Plan the installed files and identifiers — 5828 unique immutable Entry_Key values
- [x] 4. Model languages and dialects before emitting forms — 1 input language/lect IDs; registry gaps: none
- [x] 5. Emit the rich import schema — row widths {'15': 5828}; blank forms 0
- [x] 6. Parse structured linguistic information — no source-supplied grammatical labels detected by the scoped parser
- [x] 7. Build and verify the sound profile — profile route: conversion/sil-bagheli.txt; replacement characters in input forms: 0
...
- [x] 10. Produce a complete audit trail — audit: data/other/forms/raw_data/sil_bagheli_2022/manual_transcription.txt, data/other/forms/raw_data/sil_bagheli_2022/image_manifest.tsv, data/other/forms/raw_data/20260828-sil-bagheli-audit.csv, data/other/forms/raw_data/20260828-sil-bagheli-manifest.json, source_checklists/installed-record-audit.csv.gz
- [x] 11. Add focused regression tests — tests: tests/test_sil_bagheli_2022.py, tests/test_source_checklists.py
...
- Counts: 5828 installed records; 1628 compiled citation attestations.
- Exclusions: 283 Standard Hindi control occurrences, 24 non-lexical by-name occurrences, 47 blank conceptual cells, and two legible but site-unassigned response lines are audit-only; 5,828 Bagheli occurrence rows from eighteen locality lists install and yield 5,829 compiled forms after one source comma-alternative expansion.
- Unresolved cases: items 191 `berəʈɛ` and 195 `reŋeʈe` are readable but lack source site codes and remain excluded; item 189/a retains source uncertainty and item 121/l retains a medium-confidence uppercase/lowercase site-code interpretation.
- Transcription: `conversion/sil-bagheli.txt`; the 92-block OCR output is comparison evidence only, and every response line and all 3,990 conceptual cells were manually inspected.
- Validation: ten focused extraction/import/audit/profile/metadata checks pass; consolidated full build and browser QA remain deferred until the parallel survey batch lands.
- Representative app entries: Dabhaura item 1 `ɖeh`, Singpur item 16 `eŭʈʰi`, Amarkantak item 104 `beɕja`, and Domahai item 210 `ū patʃe`.
