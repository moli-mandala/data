#!/usr/bin/env python3
"""Write visually checked Bhumij Appendix B3 cells for item 190."""
...
OUT = HERE / "items_190_190_hand_keyed.tsv"
DECLARATION = "hand-keyed-from-rendered-source; OCR-not-copied"
METHOD = (
    "manual visual inspection of the 400-dpi rendered PDF page with dental "
    "marks, glottal stops, and unlabeled continuations rechecked at 800 dpi; "
...
DATA = {
    "BAI": ("imɑŋme, emɑlinɑ", "1"),
    "CHA": ("emddʒɑnɑe, imɑene", "1"),
    "DIG": ("", ""),
    "DUM": ("omɑŋme, omliɑʔɑ", "1"),
    "LAD": ("umɑŋmeʔ, omlɛd̪ɑ", "1"),
    "MAD": ("dɑ imɑŋme, omɑ dʒiɑt", "1"),
    "MOH": ("dɑ imɑŋme, omɑ dʒiɑt | ɔmem, mɑdʒije", "2 | 1"),
    "MUN": ("d̪ɑi, omɑdʒiŋe", "2"),
    "POD": ("umɑŋme, omoʔdʒiet", "1"),
    "UDA": ("d̪e emonme, omkid̪ɑ | d̪e emonme, omkid̪ɑ", "1 | 2"),
    "MCH": ("om, omɑʔɑije", "1"),
    "MDI": ("em | om", "1 | 1"),
    "MDH": ("d̪e emonme, omkid̪ɑ | d̪e emonme, omkid̪ɑ", "1 | 2"),
    "MJH": ("d̪ɑ, ɛmedʒe", "2"),
    "HDI": ("d̪ɑ, ɛmɑdʒiŋe", "2"),
    "SDI": ("em", "1"),
    "SNA": ("iŋimɛŋpe, ɛmɑd̪iŋɑj", "1"),
    "ORI": ("d̪ebɑ", "2"),
}
FIELDS = [
...
def build_rows():
    ...
    for code, (language, site, target) in SITES.items():
        ...
        row = {
            "Item": "190", "Gloss": "give!, he gave", "Site_Code": code,
...
def main():
    ...
    assert sum(len(row["Manual_Transcription"].split(" | ")) for row in rows
               if row["Review_Status"] == "attested") == 21
    assert sum(len(row["Manual_Transcription"].split(" | ")) for row in rows
               if row["Review_Status"] == "attested" and row["Target"] == "yes") == 11
