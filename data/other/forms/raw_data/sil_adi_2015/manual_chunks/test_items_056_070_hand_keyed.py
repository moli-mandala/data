...
PACKAGE = HERE.parent
LEDGER = HERE / "items_056_070_hand_keyed.tsv"
...
guard = load_module("adi_guard_items_56_70", PACKAGE / "import_adi_2015.py")
...
def test_accounting_and_form_expansion():
    rows = guard.load_manual_cells(LEDGER)
    assert sum(row["Review_Status"] == "attested" for row in rows) == 121
    assert sum(row["Review_Status"] == "source_blank" for row in rows) == 14
    assert sum(row["Review_Status"] in {"ambiguous", "illegible"}
               for row in rows) == 0
    assert len(guard.stage_forms(rows)) == 122
