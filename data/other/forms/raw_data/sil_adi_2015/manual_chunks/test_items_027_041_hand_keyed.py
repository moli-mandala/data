...
PACKAGE = HERE.parent
LEDGER = HERE / "items_027_041_hand_keyed.tsv"
...
guard = load_module("adi_guard_items_27_41", PACKAGE / "import_adi_2015.py")


def test_complete_visually_reviewed_chunk():
    rows = guard.load_manual_cells(LEDGER)
    assert len(rows) == 15 * 9 == 135
    assert len({(row["Item"], row["Site_Code"]) for row in rows}) == 135
...
def test_accounting_and_form_expansion():
    rows = guard.load_manual_cells(LEDGER)
    assert sum(row["Review_Status"] == "attested" for row in rows) == 122
    assert sum(row["Review_Status"] == "source_blank" for row in rows) == 13
    assert sum(row["Review_Status"] in {"ambiguous", "illegible"}
               for row in rows) == 0
    assert len(guard.stage_forms(rows)) == 131
