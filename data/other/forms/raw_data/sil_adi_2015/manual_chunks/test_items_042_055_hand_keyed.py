...
PACKAGE = HERE.parent
LEDGER = HERE / "items_042_055_hand_keyed.tsv"
...
guard = load_module("adi_guard_items_42_55", PACKAGE / "import_adi_2015.py")
...
def test_accounting_and_form_expansion():
    rows = guard.load_manual_cells(LEDGER)
    assert sum(row["Review_Status"] == "attested" for row in rows) == 111
